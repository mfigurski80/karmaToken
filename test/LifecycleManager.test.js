...
contract('LifecycleManager', accounts => {
  ...
  describe('service payments', () => {
    ...
    it('allows servicing bond with ether', async () => {
      ...
      // get event
      let ev = await instance
        .serviceBond(0, owner, 1, 0x0, { value: 1, from: owner })
      ...
      // Probably because beneficiary is executing and so burning gas. Check
      // recipient instead
    });
    ...
    it('allows completing bonds', async () => {
      ...
      let tx = await instance
        .serviceBond(0, owner, 10, 0x0, { value: 10, from: owner });
      // event emitted
      let ev = await getEvent(tx, 'BondServiced');
      assert.equal(ev.id, 0);
      ...
      // complete bond
      tx = await instance
        .serviceBond(0, owner, 2, 0x0, {value: 2, from: owner});
      // event emitted
    ...
    it('disallows operator without approval', async () => {
      const victim = accounts[1];
      const victimAmount = 100;
      await erc20Instance.mint(victim, victimAmount);
      await erc20Instance.approve(instance.address, victimAmount, { from: victim });
      assert.equal(
        await erc20Instance.allowance(victim, instance.address),
        victimAmount
      );
      let currencyId = await instance.addERC20Currency(erc20Instance.address)
        .then(tx => getEvent(tx, 'CurrencyAdded'))
        .then(ev => ev.id);
      const bytes = buildBondBytes({...DEFAULT_BOND, currencyRef: currencyId});
      let bondId = await instance.mintBond(bytes[0], bytes[1])
  ...
  it.skip('accepts ERC1155 single transfer as service payments', async () => {
    const currencyEvent = await instance
      .addERC1155TokenCurrency(erc1155Instance.address, 0)
      .then(tx => getEvent(tx, 'CurrencyAdded'));
    const bytes = buildBondBytes({
      ...DEFAULT_BOND,
      currencyRef: currencyEvent.id
    });
    ...
    let tx = await erc1155Instance
      .safeTransferFrom(owner, instance.address, 0, 2, 0x0);
